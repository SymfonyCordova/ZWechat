...
class StringToolkit
{
    ...
    public static function substr_html(string $content, int $length = 50, string $encoding = 'utf-8', string $ellipsis = '…'): string
    {
...
//        $content = str_replace("  ", "", $content);
        ...
        if ($length && mb_strlen(trim($content), $encoding) > $length) {
            return mb_substr(trim($content), 0, $length, $encoding) . $ellipsis;
        } else {
            return trim($content);
        }
    }
